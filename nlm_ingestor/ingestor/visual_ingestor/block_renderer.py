    ...
    def render_json(self):
        ...
        for idx, block in enumerate(self.doc.blocks):
            ...
            if 'is_table_end' in block:
                ...
                left = table_block.get("left", table_block.get('bbox', [0, 0, 0, 0])[0])
                top = table_block.get("top", table_block.get('bbox', [0, 0, 0, 0])[1])
                table_block["bbox"] = [
                        left,
                        top,
                        left + block["box_style"][3],
                        top + block["box_style"][4],
                    ]  if "box_style" in block else []                 
                table_rows = []
